...
class TestLimits(TestCase):
    ...
    def test_set_maximum(self):
        for x in range(0, 10):
            self.warnings.configure_maximum(x)
    ...
    def test_set_minimum(self):
        ...
        self.warnings.configure_maximum(11)
        for x in range(0, 10):
            self.warnings.configure_minimum(x)
            self.assertEqual(self.warnings.get_checker(DoxyChecker.name).minimum, x)

    def test_set_minimum_fail(self):
        self.warnings.configure_maximum(5)
        for x in range(1, 5):
            self.warnings.configure_minimum(x)
            self.assertEqual(self.warnings.get_checker(DoxyChecker.name).minimum, x)
    ...
    def test_return_values_maximum_decrease(self):
        ...
        self.warnings.configure_maximum(0)
    ...
    def test_return_values_maximum_increase(self):
        ...
        self.warnings.configure_maximum(2)
        self.assertEqual(self.warnings.return_check_limits(), 0)
    ...
    def test_invalid_maximum(self):
        self.warnings.configure_maximum(10)
        self.warnings.configure_minimum(10)
        with self.assertRaises(ValueError) as c_m:
